...
import { Users, UserCheck, DollarSign, AlertCircle, TrendingUp, TrendingDown, Sparkles } from 'lucide-react';
import { Skeleton } from './ui/skeleton';
import { formatINR } from '../lib/utils';
import { Button } from './ui/button';

export default function DashboardOverview() {
  const { data: metrics, isLoading: metricsLoading, error: metricsError, refetch: refetchMetrics } = useGetDashboardMetrics();
  const { data: students = [], isLoading: studentsLoading, error: studentsError, refetch: refetchStudents } = useGetAllStudents();
  const { data: teachers = [], isLoading: teachersLoading, error: teachersError, refetch: refetchTeachers } = useGetAllTeachers();
    ...
    <div className="space-y-6">
          ...
          <div className="hero-content absolute inset-0 flex items-center justify-center text-center p-6">
            <div className="animate-float">
              ...
              <p className="text-white/90 text-lg font-medium drop-shadow-md">
                Welcome back! Here's what's happening with School Mitra.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="glass-effect border-gray-200/50 dark:border-gray-800/50 shadow-premium card-hover">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 rounded-lg bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/20 dark:to-indigo-950/20 border border-blue-200/50 dark:border-blue-800/50">
                <span className="text-sm font-semibold text-gray-900 dark:text-white">Total Students</span>
                <span className="text-lg font-bold text-blue-600 dark:text-blue-400">{metrics?.totalStudents.toString() || '0'}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-950/20 border border-indigo-200/50 dark:border-indigo-800/50">
                <span className="text-sm font-semibold text-gray-900 dark:text-white">Total Teachers</span>
                <span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{metrics?.totalTeachers.toString() || '0'}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-lg bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 border border-green-200/50 dark:border-green-800/50">
                <span className="text-sm font-semibold text-gray-900 dark:text-white">Fees Collected</span>
                <span className="text-lg font-bold text-green-600 dark:text-green-400">{formatINR(metrics?.totalFeesCollected || BigInt(0))}</span>
              </div>
            </div>
          </CardContent>
        </Card>
