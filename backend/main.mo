...
actor {
  ...
  public shared ({ caller }) func addStudent(
    ...
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Runtime.trap("Unauthorized: Only teachers and admins can add students");
    };
  ...
  public shared ({ caller }) func toggleFeePaymentStatus(id : Nat) : async Bool {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Runtime.trap("Unauthorized: Only admins can toggle fee payment status");
    };
    switch (feeRecords.get(id)) {
      case (null) { Runtime.trap("Fee record not found") };
      case (?record) {
        let updatedRecord = {
          record with
          isPaid = not record.isPaid;
        };
        feeRecords.add(id, updatedRecord);
        updatedRecord.isPaid;
      };
    };
  };
